...
const meta = {
  ...
  render: (args) => {
    const [, updateArgs] = useArgs<React.ComponentProps<typeof ChapterPickerModal>>();
    const { open, current, total, available, onSelect, onClose } = args;
        ...
        {!open && (
          <div className="absolute inset-0 z-30 grid place-items-center p-6">
            <button
              ...
              className="rounded-full bg-primary px-5 py-2.5 text-sm font-semibold text-primary-foreground shadow-md hover:opacity-90"
            >
              Abrir modal (capítulo actual: {current})
            </button>
          </div>
        )}
      </>
