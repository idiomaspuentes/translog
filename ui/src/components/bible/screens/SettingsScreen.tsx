...
export function SettingsScreen({
  ...
  const [filteredLanguages, setFilteredLanguages] = useState<LanguageOption[]>(languages);
  ...
  useEffect(() => {
    ...
    const run = async () => {
      ...
      if (onSearchLanguages) {
        const result = await onSearchLanguages(query);
        if (!cancelled) setFilteredLanguages(result);
      } else {
  ...
  }, [query, languages, onSearchLanguages]);
